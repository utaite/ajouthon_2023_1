...
async def initialize_2():
    # 융합시스템공학과 - 2학년 1학기
    await Courses.create(uuid='ISE2211',
                         department=1,
                         type=1,
                         name='데이터분석프로그래밍기초',
                         credit=3,
                         time=3,
                         grade=3,
                         summary='본 교과목은 R, 매트랩을 이용한 데이터 분석 프로그래밍 기초를 다룬다. R, 매트랩 프로그래밍을 활용한 기초 숙지, 기본 계산법, 데이터 입출력, 스크립트의 활용, 그래프 작성을 포함하는 전반적인 내용을 학습한다. 이론과 실습을 통해 학생들이 주어진 문제를 해결할 수 있도록 계산 및 시각적인 표현 방법을 배운다. 데이터 분석 프로그래밍의 반복적인 실습을 통해 주어진 데이터를 효과적으로 분석할 수 있는 역량을 개발하고자 한다.',
                         prerequisite=[])

    await Courses.create(uuid='ISE202',
                         department=1,
                         type=1,
                         name='경영과학입문',
                         credit=3,
                         time=3,
                         grade=3,
                         summary='오늘날과 같이 급변하는 경영환경에서는 기업이나 공공행정 분야에서 직면하는 생산, 유통, 수송, 건설, 통신 등의 여러 가지 의사 결정 문제들을 효율적으로 해결하는 것이 필수적이다. 본 과목에서는 산업, 서비스 및 정보시스템의 운용과 관련된 여러 가지 최적화(Optimization) 문제를 이해하고, 수학적으로 정형화된 선형(Linear) 모델을 세우고 분석함으로써 최적의 대안을 제시할 수 있는 방법론을 다룬다.',
                         prerequisite=[])

    await Courses.create(uuid='ISE201',
                         department=1,
                         type=1,
                         name='기초선형대수',
                         credit=3,
                         time=3,
                         grade=3,
                         summary='행렬과 벡터에 관한 기본적인 개념을 익힌다. 특히, 행렬과 행렬식의 기본 연산법을 배우고 이를 일차방정식의 풀이에 응용하는 방법을 학습한다. 벡터의 개념, 벡터의 연산,벡터 공간에 대해서 소개하고 그 위에서의 일차변환을 행렬로 나타내는 방법을 배우고 다양한 특성들을 관찰한다. 고유값, 고유벡터를 포함한 정방행렬의 가역성 등 여러 성질들과 응용을 살펴본다.',
                         prerequisite=[])


    # 융합시스템공학과 - 2학년 2학기
    await Courses.create(uuid='ISE222',
                         department=1,
                         type=1,
                         name='시스템최적화',
                         credit=3,
                         time=3,
                         grade=4,
                         summary='본 교과목에서는 경영과학입문에서 배운 이론적 내용을 바탕으로 산업 및 정보 시스템의 운용과 관련하여 발생하는 많은 의사결정 문제들을 정형화하고 최적해를 구하는 방법들을 다룬다. 특히, 다양한 최적화 응용분야에 대한 case study를 통해 최적화 문제를 이해하고, 다양한 문제를 풀어봄으로써 문제 해결 능력을 배양한다.',
                         prerequisite=[])

    await Courses.create(uuid='ISE231',
                         department=1,
                         type=1,
                         name='스마트생산개론',
                         credit=3,
                         time=3,
                         grade=4,
                         summary='본 교과목은 스마트생산 트랙의 기초과목으로서, 스마트생산의 개념, 방법론, 기술, 발전상의 기본을 소개하여, 스마트생산 트랙에서의 추후 학습의 기반을 마련하는 것을 목적으로 한다. 전체 주제들을 종합적으로 개괄하며, 추후에 각 주제를 심도있게 학습하기 위한 안내와 관련 과목 소개도 포함한다. 수업의 주제는 다음과 같다. 스마트생산의 개념 및 배경, 관련 기술 (센서, 데이터 처리, 로봇, 소프트웨어, 3D프린팅 등), 방법론 (자동화, 무인화, 연결화, 가상화, 디지털화, 최적화 등), 형태 (시스템, 인간-기계 협동, 분산, 개인화 등), 응용 분야 및 사례: 제조, 서비스업, 물류 등 대표 사례), 영향 및 효과, 향후 발전 방향 및 새로운 비즈니스 모델',
                         prerequisite=[])

    await Courses.create(uuid='ISE221',
                         department=1,
                         type=2,
                         name='재무회계기초',
                         credit=3,
                         time=3,
                         grade=4,
                         summary='기업회계기준을 중심으로 기업회계 실무의 이론구조를 전개하는 한편, 이와 관련되는 회계관련 제 법규에 따라 회계의 실제 문제 해결에 필요한 지식을 익힌다. 재무회계 전반에 걸친 재무제표의 작성, 자산의 평가, 수익·비용의 인식 및 대응, 물가변동회계 및 기타 구체적인 회계처리 방법을 이해한다.',
                         prerequisite=[])


    # 융합시스템공학과 - 3학년 1학기
    await Courses.create(uuid='ISE316',
                         department=1,
                         type=2,
                         name='C#언어프로그래밍',
                         credit=3,
                         time=3,
                         grade=5,
                         summary='본 과목은 융합시스템공학과의 3학년을 대상으로 하는 전공선택으로서 컴파일링 언어인 C#에 대한 개념과 문법을 강의한다. 본 과목에서 지향하는 1차적인 목표는 하향식(top-down) 프로그램 설계에 따른 구조적 프로그래밍의 실현이다. 수강생은 본인의 아이디어를 규모가 작고 다루기 쉬운 단위 작업들로 쪼개어 최소한의 단위 모듈로 구현해 나간다. 2차적인 목표는 객체지향 방식을 도입한 C# 언어의 장점인 컨퍼넌트 기반의 모듈화된 프로그램의 주요 기능을 이해하는 것이다. 객체지향적으로 프로그램을 설계하기 위해서, 다루어야 할 객체를 정확히 서술할 수 있는 클래스의 개념을 도입하고 이를 수강생들이 C#의 문법적 의미를 정확하게 전달한다. 이것은 1차적인 목표의 하향식 설계 방법에서 확장하는 상향식(bottom-up) 프로그램을 개발할 수 있는 능력을 개발하는 것이다. 수강생들은 본 과목을 통해 이러한 구조적, 절차적인 언어의 개념과 객체지향 방식을 이해하고, 현업의 실무에서 활용할 수 있는 수준으로 프로그래밍 능력을 향상시키는 것을 목적으로 한다.',
                         prerequisite=[])

    await Courses.create(uuid='ISE335',
                         department=1,
                         type=2,
                         name='제조정보시스템분석설계',
                         credit=3,
                         time=3,
                         grade=5,
                         summary='본 교과목은 제조공정제어의 기본 개념, 기술, 실행 방법을 소개하고 기초 프로그래밍을 습득하는 것을 목표로 한다. 또한 PLC, 자동화장비, 센서의 인터페이스에서 나타나는 분산제어시스템의 여러 주제를 개괄한다. 제조공정 제어에서 대두되는 네트워크와 정보 전달, 생산수행시스템과의 연동 문제도 다루어, 스마트 제조의 필수 요소인 공정제어에 대한 이해를 증진시킨다. 수업의 주제는 다음과 같다. - 공정제어 로직 및 활동에 대한 이해 - 공정제어 기술 및 장치에 대한 이해 - 제어 프로그래밍(PLC) 기초 습득 - 제어시스템 설계 원리 이해 - 제어네트워크 설계 원리 이해 - 모니터링 시스템 원리 이해',
                         prerequisite=[])

    # 융합시스템공학과 - 3학년 2학기
    await Courses.create(uuid='ISE322',
                         department=1,
                         type=2,
                         name='물류SCM',
                         credit=3,
                         time=3,
                         grade=6,
                         summary='정보컴퓨터기술의 발전으로 조직의 모든 업무가 정보화됨에 따라 제품의 물리적인 수송, 하역, 저장 등의 물류가 오히려 가장 취약하고 중요한 업무가 되고 있다. 본 과목은 기업내, 기업간 및 국가간 발생하는 물류업무를 저비용, 고효율, 단시간화하기 위해 레이아웃, 자재관리, 자재취급(material handling), 창고/물류센터의 설계 및 운영, 수배송, 물류포장, 수출입 물류, 물류정보시스템, 물류표준화, 물류코스트, SCM(Supply Chain Management) 및 성과 평가 등의 주제를 다룬다.',
                         prerequisite=[])

    await Courses.create(uuid='ISE301',
                         department=1,
                         type=2,
                         name='기술글쓰기입문',
                         credit=3,
                         time=3,
                         grade=6,
                         summary='본 교과목은 과학적, 기술적 내용과 의사를 문서로 원활히 전달할 수 있는 능력을 배양하는 것을 목표로 한다. 수업의 주제는 기술문서의 분류와 특성, 명확한 글쓰기를 위한 기술 글쓰기 원리, 숫자와 데이터 관련 글쓰기, 기호와 단위 표현, 시각자료 서술, 과정 서술 방법, 주장과 논증 서술 방법, 협력 글쓰기 도구, 교정과 퇴고 방법, 기술문서 스타일 종류, 스타일 지침서 참조 방법, 대표적인 기술문서 종류별 글쓰기 방법 등을 포함한다. 본 과목 수강을 통해 기술문서 작성에서의 명확한 의미 전달, 정확한 정보 전달, 증거 기반 논리적 설득을 위한 기초를 습득할 수 있다. 수업은 이론 강의와 학생의 글에 대한 비평, 토론, 개별지도로 진행된다.',
                         prerequisite=[])


    # 불어불문학과 - 3학년 1학기
    await Courses.create(uuid='FRE328',
                         department=2,
                         type=1,
                         name='프랑코포니 문학과 사회',
                         credit=3,
                         time=3,
                         grade=5,
                         summary='프랑스어권 아프리카, 북미, 카리브해 지역의 언어와 문학을 연구한다. 네그리튀드 문학, 크레올 문학, 퀘벡 문학의 특성을 알아본다.',
                         prerequisite=[])

    await Courses.create(uuid='FRE3117',
                         department=2,
                         type=2,
                         name='시사프랑스어 독해',
                         credit=3,
                         time=3,
                         grade=5,
                         summary='프랑스를 비롯한 전 세계의 주요 시사들을 인터넷, 방송, 신문 등 다양한 매스미디어에서 발췌한 텍스트들을 독해하는 과정을 통해 알아본다.',
                         prerequisite=[])

    await Courses.create(uuid='FRE3114',
                         department=2,
                         type=2,
                         name='고급프랑스어 회화',
                         credit=3,
                         time=3,
                         grade=5,
                         summary='고급수준의 프랑스어 회화 능력을 함양하기 위한 심화수업을 진행한다.',
                         prerequisite=[])

    # 불어불문학과 - 3학년 2학기
    await Courses.create(uuid='FRE3122',
                         department=2,
                         type=1,
                         name='프랑코포니 언어학',
                         credit=3,
                         time=3,
                         grade=6,
                         summary='프랑코포니 전역에서 사용되는 다양한 프랑스어의 변이형들, 퀘벡 프랑스어, 크레올어, 벨기에의 왈로니어, 마그렙 및 사하라 이남의 서부 아프리카에서 사용되는 프랑스어 대해 공부한다.',
                         prerequisite=[])

    await Courses.create(uuid='FRE3121',
                         department=2,
                         type=1,
                         name='프랑스어활용연습(캡스톤디자인)',
                         credit=3,
                         time=3,
                         grade=6,
                         summary='본 과목은 1~3학년 과정에서 습득한 프랑스어 기본 능력을 바탕으로, 구직, 취업, 그 밖의 일상생활에 적용 가능한 실용적 언어 능력을 제고하는 것을 목표로 한다. 은행, 관공서 업무 등 실생활 속에서 필요한 프랑스어, 프리젠테이션에 필요한 프랑스어, 구직 및 직업 생활(이력서 작성, 면접, 전화, 메일, 회의, 거래, 협상 등)에 필요한 프랑스어 등, 실제 상황별로 필요한 언어 능력을 갖추도록 수업을 진행할 것이다. 이 수업은 최소 B1 이상 수준을 갖춘 학생들에게 권장된다.',
                         prerequisite=[])

    await Courses.create(uuid='FRE325',
                         department=2,
                         type=2,
                         name='프랑스 시와 상상력',
                         credit=3,
                         time=3,
                         grade=6,
                         summary='19세기 낭만주의에서 20세기 초현실주의 이후 현대까지 전개된 프랑스 시문학 전반에 대하여 살펴본다. 문예사조별로 중요한 시인과 작품들을 공부하면서 상상력, 시적 감성, 언어, 시와 샹송의 관계에 대해 알아본다.',
                         prerequisite=[])

    await Courses.create(uuid='FRE3317',
                         department=2,
                         type=2,
                         name='프랑스 사회와 기업',
                         credit=3,
                         time=3,
                         grade=6,
                         summary='본 강의에서는 프랑스의 대표적인 기업들, 특히 세계 최고의 기술력을 보유하고 영향력을 행사하는 기업들을 대상으로 기술과 경영 문화를 중심으로 한 역사를 살펴본다. 프랑스는 17세기 말부터 현재에 이르기까지 전 세계 명품 산업에서 1등 국가의 자리를 지키고 있다. 세계사적으로 하나의 국가가 특정한 산업 분야에서, 그것도 패션, 화장품, 향수, 보석, 실크, 헤어디자인, 유리/거울/렌즈, 요리, 제과-제빵, 와인, 관광 등 고부가 가치 산업에서 최고의 지위를 350여 년 이상의 기간 동안 누리는 예가 없다. 도대체 그 기원이 무엇이고 비결이 무엇일까? 또한 프랑스는 Michelin 같은 타이어 제조업체와 Airbus 같은 항공 산업, 그리고 방위산업 및 AXA, BNP Paribas와 같은 금융업 등 이외의 다양한 산업 분야에서 세계를 주도하고 있다. 이 강의에서는 이 같은 산업의 역사를 살펴보고 이로부터 우리는 무엇을 해야할지 함께 알아보기로 한다.',
                         prerequisite=[])

    # 불어불문학과 - 4학년 1학기
    await Courses.create(uuid='FRE439',
                         department=2,
                         type=2,
                         name='프랑코포니 예술',
                         credit=3,
                         time=3,
                         grade=7,
                         summary='본 과목은 프랑스어권 지역에서 이루어지는 다양한 예술 활동을 대상으로 하여, 여러 예술 작품을 감상하고 주요 예술가와 예술 경향을 이해하는 것을 1차적인 목표로 한다. 예술 작품 혹은 예술 현상 및 이에 대한 텍스트를 함께 접함으로써 예술을 감상하고 예술에 대해 논평할 수 있는 능력을 기르고자 한다. 끝으로 이러한 학습과정을 통해 예술과 사회의 관계에 대한 시각을 갖추는 것을 목표로 한다. 예술과 문화 분야로 진출하고자 하는 학생들에게는 현대 문화예술계의 흐름을 파악할 수 있는 능력을 길러 줄 것이며, 프랑코포니 분야로 진출하고자 하는 학생들에게는 해당 사회에 대한 보다 깊은 이해를 가능하게 해줄 것이다. 프랑코포니 입문을 미리 수강한 후에 신청하는 것을 권장한다.',
                         prerequisite=[])

    await Courses.create(uuid='FRE4311',
                         department=2,
                         type=2,
                         name='유럽 문화와 콘텐츠',
                         credit=3,
                         time=3,
                         grade=7,
                         summary='본과목은 문화관광, 문화유산 콘텐츠와 관련하여 유럽연합 및 서유럽의 사례를 연구하고 시사점을 이끌어내는 것을 목표로 한다. 문화 유산의 활용, 문화유산을 통한 문화 관광 콘텐츠 개발, 공간의 특성을 활용한 콘텐츠 개발 등에 대한 지식을 습득하고 관련 사례를 분석 및 비판적으로 설명하는 연습을 한다.',
                         prerequisite=[])


    # 불어불문학과 - 4학년 2학기
    await Courses.create(uuid='FRE424',
                         department=2,
                         type=2,
                         name='현대 프랑스 문화',
                         credit=3,
                         time=3,
                         grade=8,
                         summary='미술, 연극, 문학, 거리예술, 서커스, 패션, 요리, 만화, 건축, 무용, 축제 등 현대프랑스 문화예술의 다양한 지형과 흐름을 작품이나 사건들을 중심으로 파악한다.',
                         prerequisite=[])
